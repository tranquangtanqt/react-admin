...
"column_5" DATE NOT NULL ENABLE`);
  ...
  const getAllColumn = () => {
    ...
    for (let j = 0; j < columnArr.length; j++) {
      const arr = columnArr[j].trim().split(' ');
    ...
    if (Number(end)) {
      for (let i = Number(start); i <= Number(end); i++) {
        ...
        for (let j = 0; j < row.length; j++) {
          ...
          const columnName = StringUtils.replaceAll(
            arrSeparatorSpace[0],
            `"`,
            '',
          );
          ...
          if (rowString !== '') {
            ...
            if (rowString.toLowerCase().includes('default')) {
              ...
              value =
                arrSeparatorSpace[
                  arrSeparatorSpaceLower.indexOf('default') + 1
                ];
              outputStr += '' + value + ', ';
              ...
              if (limitArr.length > 1) {
                const natural = Math.floor(
                  Math.random() *
                    Math.pow(10, Number(limitArr[0]) - Number(limitArr[1])),
                );
                const decimal = Math.floor(
                  Math.random() * Math.pow(10, Number(limitArr[1])),
                );
                value = natural + '.' + decimal;
                outputStr += '' + value + ', ';
              } else if (limitArr.length > 0) {
                const natural = Math.floor(
                  Math.random() * Math.pow(10, Number(limitArr[0])),
                );
                outputStr += '' + natural + ', ';
              }
