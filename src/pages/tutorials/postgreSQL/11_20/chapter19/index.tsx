...
export const TutorialPostgreSQLChapter19 = () => {
  return (
    <>
      <PageTitle title="19. Role Management"></PageTitle>
      <div className="row mt-2">
        <div className="col-12 col-sm-12 col-md-12">
          <b>1. Create a user with a password</b>
          <div className="tab-1">
            <p>
              Generally you should avoid using the default database role (often postgres) in your application. You should instead create a user with
              lower levels of privileges. Here we make one called niceusername and give it a password verystrong-PASSWORD
            </p>
          </div>
          <div className="tab-2">
          ...
          <div className="tab-1">
            <p>
              The problem with that is that queries typed into the psql console get saved in a history file .psql_history in the user's home directory
              and may as well be logged to the PostgreSQL database server log, thus exposing the password.
            </p>
            <p>
              To avoid this, use the \PASSWORD command to set the user password. If the user issuing the command is a superuser, the current password
              will not be asked. (Must be superuser to alter passwords of superusers)
            </p>
          </div>
          <div className="tab-2">
            <Code
...
ALTER DEFAULT PRIVILEGES IN SCHEMA myschema GRANT ALL ON TABLES TO ADMIN;`}
          ...
          <div className="tab-2">
            <Code code={`ALTER DEFAULT PRIVILEGES FOR ROLE ADMIN GRANT SELECT ON TABLES TO read_only;`} language="sql" />
          </div>
        </div>
      </div>
    </>
