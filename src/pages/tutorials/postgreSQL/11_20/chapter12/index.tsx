...
(8 rows)
    `}
          ...
          <div className="tab-2">
            <ul>
              <li>
                dist_by_i: DENSE_RANK() OVER (ORDER BY i) is like a row_number per distinct values. Can be used for the number of distinct values of i
                (COUNT(DISTINCT i) wold not work). Just use the maximum value.
              </li>
              <li>prev_t: LAG(t) OVER () is a previous value of t over the whole window. mind that it is null for the first row</li>
              <li>nth: NTH_VALUE(i, 6) OVER () is the value of sixth rows column i over the whole window</li>
              <li>num_by_i: COUNT(TRUE) OVER (PARTITION BY i) is an amount of rows for each value of i</li>
              <li>num_all: COUNT(TRUE) OVER () is an amount of rows over a whole window</li>
              <li>ntile: NTILE(3) over() splits the whole window to 3 (as much as possible) equal in quantity parts</li>
            </ul>
          </div>
        </div>
      </div>
...
FROM wf_example`}
          ...
          <div className="tab-1">
            <ul>
              <li>
                dense_rank orders VALUES of i by appearance in window. i=1 appears, so first row has dense_rank, next and third i value does not
                change, so it is dense_rank shows 1 - FIRST value not changed. fourth row i=2, it is second value of i met, so dense_rank shows 2,
                andso for the next row. Then it meets value i=3 at 6th row, so it show 3. Same for the rest two values of i. So the last value of
                dense_rank is the number of distinct values of i
              </li>
              <li>row_number orders ROWS as they are listed</li>
              <li>
                rank Not to confuse with dense_rank this function orders ROW NUMBER of i values. So it starts same with three ones, but has next value
                4, which means i=2 (new value) was met at row 4. Same i=3 was met at row 6. Etc..
              </li>
            </ul>
          </div>
        </div>
